...
async function fetchPageMeta(url: string): Promise<{ title: string; description: string }> {
  ...
  try {
    ...
    await new HTMLRewriter()
      .on('title', { text(t) { if (t.text) meta.title = (meta.title + t.text).slice(0, 200) } })
      .on('meta[name="description"]', { element(e) { meta.description ||= e.getAttribute('content')?.slice(0, 500) || '' } })
      .on('meta[property="og:title"]', { element(e) { meta.title ||= e.getAttribute('content')?.slice(0, 200) || '' } })
      .on('meta[property="og:description"]', { element(e) { meta.description ||= e.getAttribute('content')?.slice(0, 500) || '' } })
      .transform(resp).text()
  } catch {}
  return meta
}
...
app.get('/api/bookmarks', async (c) => {
  ...
  if (q) { conditions.push(`(b.title LIKE ? OR b.summary LIKE ? OR b.note LIKE ?)`); params.push(`%${q}%`, `%${q}%`, `%${q}%`) }
  if (domain) { conditions.push(`b.domain = ?`); params.push(domain) }
  ...
  if (ids.length > 0) {
    ...
    for (const row of tagRows) {
      ...
      tagMap[bid].push({ id: row.id, name: row.name, slug: row.slug, source: row.source, confidence: row.confidence })
    }
  }

  return c.json({ ok: true, data: results.map(r => ({ ...r, tags: tagMap[r.id as string] || [] })) })
})
...
app.post('/api/bookmarks', async (c) => {
  ...
  const { results: [dup] } = await db.prepare(`SELECT id, url, title FROM bookmarks WHERE canonical_url = ?`).bind(canonical).all()
  if (dup) return c.json({ ok: false, error: 'bookmark already exists', existing: dup }, 409)

  if (tagIds.length > 0) {
    const { results: validTags } = await db.prepare(`SELECT id FROM tags WHERE id IN (${tagIds.map(() => '?').join(',')})`).bind(...tagIds).all<{ id: string }>()
    const validIds = new Set(validTags.map(t => t.id))
    const invalid = tagIds.filter(id => !validIds.has(id))
    if (invalid.length > 0) return c.json({ ok: false, error: `invalid tag ids: ${invalid.join(', ')}` }, 400)
  }

  await db.prepare(
    `INSERT INTO bookmarks (id, url, canonical_url, title, domain, summary, note, type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, rawUrl, canonical, body.title || rawUrl, extractDomain(rawUrl), body.summary || '', body.note || '', body.type || 'other', ts, ts).run()

  for (const tagId of tagIds) {
    await db.prepare(`INSERT OR IGNORE INTO bookmark_tags (bookmark_id, tag_id, source, confidence, status, created_at) VALUES (?, ?, 'user', NULL, 'active', ?)`)
...
app.put('/api/bookmarks/:id', async (c) => {
  ...
  const db = c.env.DB
  const ts = now()
  const { results: [existing] } = await db.prepare(`SELECT id, title FROM bookmarks WHERE id = ?`).bind(id).all<{ id: string; title: string }>()
  if (!existing) return c.json({ ok: false, error: 'bookmark not found' }, 404)

  await db.prepare(`UPDATE bookmarks SET title=?, summary=?, note=?, type=?, updated_at=? WHERE id=?`)
...
app.get('/api/tags', async (c) => {
  ...
  const params: string[] = []
  if (q) { sql += ` WHERE t.name LIKE ? OR t.slug LIKE ?`; params.push(`%${q}%`, `%${q}%`) }
...
app.post('/api/tags', async (c) => {
  ...
  const db = c.env.DB
  const { results: [existing] } = await db.prepare(`SELECT * FROM tags WHERE slug = ?`).bind(slug).all<{ id: string }>()
  if (existing) {
    const { results: [tag] } = await db.prepare(
  ...
  await db.prepare(`INSERT INTO tags (id, name, slug, created_at, updated_at) VALUES (?, ?, ?, ?, ?)`)
    .bind(id, (body.name as string).trim(), slug, ts, ts).run()
...
app.put('/api/tags/:id', async (c) => {
  ...
  const ts = now()
  await db.prepare(`UPDATE tags SET name=?, slug=?, updated_at=? WHERE id=?`).bind((body.name as string).trim(), newSlug, ts, id).run()
